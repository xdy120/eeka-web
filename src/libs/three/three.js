...
THREE.Vector3.prototype = {
  set: function (a, c, b) {
    ...
    var a = 0.0001;
    return (Math.abs(this.x) < a) && (Math.abs(this.y) < a) && (Math.abs(this.z)
        < a)
  }, clone: function () {
...
THREE.Rectangle = function () {
  ...
  this.instersects = function (i) {
    return Math.min(d, i.getX2()) - Math.max(f, i.getX1()) >= 0 && Math.min(g,
        i.getY2()) - Math.max(h, i.getY1()) >= 0
  };
  this.empty = function () {
    e = true;
    f = 0;
...
THREE.Matrix4.prototype = {
  ...
  multiplySelf: function (c) {
    var o = this.n11, n = this.n12, k = this.n13, i = this.n14, f = this.n21,
        e = this.n22, d = this.n23, b = this.n24, a = this.n31, r = this.n32,
        q = this.n33, p = this.n34, l = this.n41, j = this.n42, h = this.n43,
        g = this.n44;
...
THREE.Matrix4.rotationAxisAngleMatrix = function (b, d) {
  var a = new THREE.Matrix4(), f = Math.cos(d), j = Math.sin(d), i = 1 - f,
      h = b.x, g = b.y, e = b.z;
  a.n11 = i * h * h + f;
  a.n12 = i * h * g - j * e;
  a.n13 = i * h * e + j * g;
...
THREE.Matrix4.makeInvert3x3 = function (o) {
  var e = o.flatten(), l = new THREE.Matrix3(), n = e[10] * e[5] - e[6] * e[9],
      i = -e[10] * e[1] + e[2] * e[9], d = e[6] * e[1] - e[2] * e[5],
      k = -e[10] * e[4] + e[6] * e[8], g = e[10] * e[0] - e[2] * e[8],
      c = -e[6] * e[0] + e[2] * e[4], j = e[9] * e[4] - e[5] * e[8],
      f = -e[9] * e[0] + e[1] * e[8], a = e[5] * e[0] - e[1] * e[4],
      h = e[0] * (n) + e[1] * (k) + e[2] * (j), b;
  if (h == 0) {
...
THREE.Object3D = function (a) {
  ...
  this.updateMatrix = function () {
    ...
    this.matrixRotation.multiplySelf(
        THREE.Matrix4.rotationYMatrix(this.rotation.y));
    this.matrixRotation.multiplySelf(
        THREE.Matrix4.rotationZMatrix(this.rotation.z));
...
THREE.Projector = function () {
  var e = null, c, p, n = [], b, f, l = [], k, m, i = [], j, h, a = [],
      g = new THREE.Vector4(), d = new THREE.Matrix4(), o = new THREE.Matrix4();
        ...
        for (I = 0, B = w.length; I < B; I++) {
          ...
          if (A instanceof THREE.Face3) {
            ...
            if (y.__visible && x.__visible && t.__visible) {
              if ((L.doubleSided || (L.flipSided != (t.positionScreen.x
                  - y.positionScreen.x) * (x.positionScreen.y
                  - y.positionScreen.y) - (t.positionScreen.y
                  - y.positionScreen.y) * (x.positionScreen.x
                ...
                c.z = Math.max(y.positionScreen.z,
                    Math.max(x.positionScreen.z, t.positionScreen.z));
            ...
            if (A instanceof THREE.Face4) {
              ...
              if (y.__visible && x.__visible && t.__visible && s.__visible) {
                if ((L.doubleSided || (L.flipSided != ((s.positionScreen.x
                    - y.positionScreen.x) * (x.positionScreen.y
                    - y.positionScreen.y) - (s.positionScreen.y
                    - y.positionScreen.y) * (x.positionScreen.x
                    - y.positionScreen.x) < 0 || (x.positionScreen.x
                    - t.positionScreen.x) * (s.positionScreen.y
                    - t.positionScreen.y) - (x.positionScreen.y
                    - t.positionScreen.y) * (s.positionScreen.x
                    - t.positionScreen.x) < 0)))) {
                  ...
                  b.z = Math.max(y.positionScreen.z,
                      Math.max(x.positionScreen.z,
                          Math.max(t.positionScreen.z, s.positionScreen.z)));
        ...
        if (L instanceof THREE.Line) {
          ...
          if (L instanceof THREE.Particle) {
            ...
            if (L.screen.z > 0 && L.screen.z < 1) {
              ...
              j.scale.x = L.scale.x * Math.abs(g.x / g.w - (g.x
                  + G.projectionMatrix.n11) / (g.w + G.projectionMatrix.n14));
              j.scale.y = L.scale.y * Math.abs(g.y / g.w - (g.y
                  + G.projectionMatrix.n22) / (g.w + G.projectionMatrix.n24));
...
THREE.DOMRenderer = function () {
  ...
  var e = null, g = new THREE.Projector(), b = document.createElement("div"), a,
      c, f, d;
  this.domElement = b;
...
THREE.CanvasRenderer = function () {
  var A = null, v = new THREE.Projector(), s = document.createElement("canvas"),
      n, r, k, p, t = s.getContext("2d"), D = new THREE.Rectangle(),
      q = new THREE.Rectangle(), z = new THREE.Rectangle(), l = false,
      o = new THREE.Color(4294967295), x = new THREE.Color(4294967295),
      f = new THREE.Color(4294967295), y = Math.PI * 2, j = new THREE.Vector2(),
      h = new THREE.Vector3(), G = new THREE.UV(), F = new THREE.UV(),
      E = new THREE.UV(), C = new THREE.UV(), d = new THREE.Vector2(),
      b = new THREE.Vector2();
  this.domElement = s;
  this.autoClear = true;
  this.setSize = function (I, H) {
...
THREE.SVGRenderer = function () {
  var w = null, r = new THREE.Projector(),
      s = document.createElementNS("http://www.w3.org/2000/svg", "svg"), k, A,
      p, b, y = new THREE.Rectangle(), v = new THREE.Rectangle(), i = false,
      l = new THREE.Color(4294967295), u = new THREE.Color(4294967295),
      c = new THREE.Color(4294967295), g = new THREE.Vector3(), d = [], m = [],
      B, o, f, z = 1;
  this.domElement = s;
  this.autoClear = true;
  ...
  function h(E, D, C, K, J, I, F, H, G) {
    B = n(o++);
    B.setAttribute("d", "M " + E + " " + D + " L " + C + " " + K + " L " + J
        + "," + I + "z");
    if (H instanceof THREE.MeshColorFillMaterial) {
      ...
      if (H instanceof THREE.MeshColorStrokeMaterial) {
        ...
        B.setAttribute("style", "fill: none; stroke: " + l.__styleString
  ...
  function e(G, E, C, M, L, K, F, D, H, J, I) {
    B = n(o++);
    B.setAttribute("d", "M " + G + " " + E + " L " + C + " " + M + " L " + L
        + "," + K + " L " + F + "," + D + "z");
...
THREE.WebGLRenderer = function () {
  var j = document.createElement("canvas"), c, e, l = new THREE.Matrix4(), i,
      a = 0, k = 1, h = 2, d = 3;
  this.domElement = j;
  this.autoClear = true;
  g();
  ...
  this.createBuffers = function (F, D) {
    var B, t, v, s, A, E, r, p, o, n, m, q = F.materialFaceGroup[D], x = [],
        z = [], w = [], C = [], y = [], u = 0;
  ...
  this.renderBuffer = function (n, m) {
    if (n instanceof THREE.MeshPhongMaterial) {
      ...
      c.uniform4f(e.mSpecular, mSpecular.r, mSpecular.g, mSpecular.b,
          n.opacity);
      ...
      if (n instanceof THREE.MeshColorFillMaterial) {
        color = n.color;
        c.uniform4f(e.uniformColor, color.r * color.a, color.g
            * color.a, color.b * color.a, color.a);
    ...
    if (n instanceof THREE.MeshBitmapMaterial || n
        instanceof THREE.MeshColorFillMaterial || n
  ...
  function f() {
    e = c.createProgram();
    c.attachShader(e, b("fragment",
        ["#ifdef GL_ES", "precision highp float;", "#endif",
          "uniform sampler2D diffuse;", "uniform vec4 uniformColor;",
          "varying vec2 vertexUv;", "varying vec3 lightWeighting;",
          "varying vec3 vNormal;", "uniform int material;",
          "uniform vec4 mAmbient;", "uniform vec4 mDiffuse;",
          "uniform vec4 mSpecular;", "uniform float mShininess;",
          "varying vec3 pLightVectorPoint;",
          "varying vec3 pLightVectorDirection;", "varying vec3 pViewPosition;",
          "void main(){", "if(material==3) { ",
    ...
    c.attachShader(e, b("vertex",
        ["attribute vec3 position;", "attribute vec3 normal;",
          "attribute vec2 uv;", "uniform bool enableLighting;",
          "uniform vec3 ambientColor;", "uniform vec3 directionalColor;",
          "uniform vec3 lightingDirection;", "uniform vec3 pointColor;",
          "uniform vec3 pointPosition;", "uniform mat4 objMatrix;",
          "uniform mat4 viewMatrix;", "uniform mat4 modelViewMatrix;",
          "uniform mat4 projectionMatrix;", "uniform mat3 normalMatrix;",
          "varying vec4 vertexColor;", "varying vec2 vertexUv;",
          "varying vec3 lightWeighting;", "varying vec3 vNormal;",
          "varying vec3 pLightVectorPoint;",
          "varying vec3 pLightVectorDirection;", "varying vec3 pViewPosition;",
          "uniform vec3 cameraPosition;", "void main(void) {",
          ...
          "if(!enableLighting) {", "lightWeighting = vec3(1.0, 1.0, 1.0);",
          "} else {",
          ...
          "}", "vNormal = transformedNormal;", "vertexUv = uv;",
