    ...
    <el-container>
      ...
      <el-main>
        <el-table :data="detailList" height="400px" show-summary>
          ...
          <el-table-column label="唯品会箱码">
            <template slot-scope="scope">
              <el-input v-if="canEdit(scope.row)" v-model="scope.row.boxNumber"></el-input>
              <span v-else>{{scope.row.boxNumber}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="唯品规格编码" prop="vipBarcode"></el-table-column>
          <el-table-column label="PO编码">
            <template slot-scope="scope">
              <el-input v-if="canEdit(scope.row)" v-model="scope.row.poCode"></el-input>
              <span v-else>{{scope.row.poCode}}</span>
            </template>
          </el-table-column>
          <go-quantity-column label="退货数量" prop="returnQuantity">
            <template slot-scope="scope">
              <el-input v-if="canEdit(scope.row)" v-model="scope.row.returnQuantity"></el-input>
          ...
          <el-table-column label="操作" min-width="150">
            <template slot-scope="scope">
              <go-modify-button v-if="editMode && scope.row.vipReturnDetailId"
                                @click="editRow(scope.row)"></go-modify-button>
              <go-confirm-button v-if="canEdit(scope.row)  && scope.row.vipReturnId"
                                 @click="modify(scope.row)"></go-confirm-button>
              <go-cancel-button v-if="canEdit(scope.row) && scope.row.vipReturnDetailId"
                                @click="uneditRow(scope.row)"></go-cancel-button>
              <go-delete-button v-if="editMode && scope.row.vipReturnDetailId"
...
<script>
  ...
  import {MustColumns, SkuColumns, VipReturnDetailApi} from './api';
  ...
  export default {
    ...
    data() {
      return {
        api: VipReturnDetailApi,
        pk: 'vipReturnDetailId',
        orderId: 'vipReturnId',
        columns: SkuColumns,
        relation: SkuColumns,
