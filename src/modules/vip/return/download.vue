<template>
  <el-dialog title="下载订单" :visible.sync="visible" width="450px">
    <el-form :model="domain" :rules="rules" ref="editForm" label-width="110px">
      <el-form-item>
        <el-radio v-model="downloadType" label="outerCode">退供单号</el-radio>
        <el-radio v-model="downloadType" label="createTime">生成时间</el-radio>
      </el-form-item>
      <el-form-item label="店铺" prop="storeId">
        <store-selector v-model="domain.storeId"></store-selector>
      </el-form-item>
      <el-form-item v-if="downloadType==='outerCode'" label="退供单号" prop="outerCode">
        <el-input v-model="domain.outerCode" type="textarea" :rows="5"></el-input>
      </el-form-item>
      <div v-else>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      doSave() {
        VipReturnApi.download(this.domain);
