        ...
        <el-table :data="detailList" height="400px" show-summary>
          ...
          <el-table-column prop="planQuantity" label="计划数量" sortable>
            <template slot-scope="scope">
              <div v-if="!canEdit(scope.row) && scope.row.vipAdjustDetailId">
          ...
          <el-table-column label="操作" min-width="150" fixed="right">
            <template slot-scope="scope">
              <go-modify-button v-if="editMode && scope.row.vipAdjustDetailId"
                                @click="editRow(scope.row)"></go-modify-button>
              <go-confirm-button v-if="canEdit(scope.row)  && scope.row.vipAdjustId"
                                 @click="modify(scope.row)"></go-confirm-button>
              <go-cancel-button v-if="canEdit(scope.row) && scope.row.vipAdjustDetailId"
